...
(define (%alist->style pair)
  (let ((key (car pair))
        (value (cadr pair)))
    (cons (symbol->string key) value)))

(define (alist->style alist)
  (map %alist->style alist))

(define (make-prop attr mc)
  (let ((key (symbol->string (car attr)))
        (value (cadr attr)))
    (cond
     ((string=? key "style") (cons "style" (alist->style value)))
     ((string=? (substring key 0 2) "on")
...
(define (on-user-set model event)
  (define name (webui-event-target-value event))
  (set! db (nstore-delete db space (vector "user" "name" (pk (username)))))
  (set! db (nstore-add db space (vector "user" "name" name)))
  model)

(define (generator-map->list proc g)
  (let loop ((out '()))
    (let ((item (g)))
      (if (eof-object? item)
          (reverse out)
          (loop (cons (proc item) out))))))

(define (username)
  (define out
    (generator->list-reversed
     (gmap (lambda (a) (ref a 'username))
           (nstore-from db space (vector "user" "name" (var 'username))))))
  (if (null? out) "future-schemer" (car out)))

(define (on-sign-in model event)
  (username)
  'search)
...
(define (view-login)
  `(div (@ (style ((display "flex")
        ...
        (div (@ (style ((textAlign "center"))))
             ...
             (input (@ (class "box")
                       (type "text")
                       (placeholder "future-schemer")
                       (on-change ,on-user-set)))
             (input (@ (class "box")
                       (type "submit")
                       (value "sign-in")
                       (on-click ,on-sign-in))))))

(define (on-view-search model event)
  'search)

(define (on-query-set model event)
  model)

(define (on-query-submit model event)
  model)

(define (on-view-incoming model event)
  'incoming)

(define (on-view-convo model event)
  'convo)
...
(define (view-search)
  `(div (@ (style ((padding "15px 15px")
                   (margin "auto")
                   (width "888px"))))
        (div (@ (class "box")) "Welcome " ,(username) " to babelia! · "
             (span (@ (style ((fontWeight "bold")))) "search") " · "
             (span (@ (on-click ,on-view-incoming))"incoming") " · "
             (span (@ (on-click ,on-view-convo)) "convo") " · ")
        (input (@ (class "box")
                  (type "text")
                  (placeholder "query")
                  (style ((width "100%")))
                  (on-change ,on-query-set)))
        (input (@ (class "box")
                  (type "submit")
                  (value "submit")
                  (style ((width "100%")))
                  (on-click ,on-query-submit)))
        (div (@ (class "box"))
             (p (@ (style ((color "lightblue") (fontWeight "bold")))) "scheme-lang.com")
             (p (@ (style ((lineHeight "1.2em") (marginTop "10px") (marginBottom "10px"))))
...
restrictions that make additional features appear necessary.")
             (p (@ (style ((color "lightgreen"))))
                "https://scheme-lang.com/"))
        (div (@ (class "box"))
             (p (@ (style ((color "lightblue") (fontWeight "bold")))) "pre-srfi/webui/todomvc")
             (p (@ (style ((lineHeight "1.2em") (marginTop "10px") (marginBottom "10px"))))
                "Classic todomvc application done with Gambit Scheme and ReactJS")
             (p (@ (style ((color "lightgreen"))))
                "https://pre-srfi.github.io/webui/demos/03-todomvc/"))
        (div (@ (class "box"))
             (p (@ (style ((color "lightblue") (fontWeight "bold")))) "pre-srfi/webui/convo")
             (p (@ (style ((lineHeight "1.2em") (marginTop "10px") (marginBottom "10px"))))
                "Wanna-be nlp / nlu user interface for a deep-thought chatbot!")
             (p (@ (style ((color "lightgreen"))))
                "https://pre-srfi.github.io/webui/demos/04-convo/"))))

(define (view model)
  (case model
    ((login) (view-login))
    ((search) (view-search))
    (else (raise 'unknown-route))))
